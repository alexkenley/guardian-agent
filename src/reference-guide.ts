            ...
            sections: [
              ...
              },
              {
                ...
                items: [
                  ...
                  'Use the Guardian-native browser wrapper tools first: `browser_capabilities`, `browser_navigate`, `browser_read`, `browser_links`, `browser_extract`, and `browser_interact`.',
                  'The wrapper treats browser work as a hybrid lane: Lightpanda is preferred for DOM-first reads and structured extraction, while Playwright is used for interactive actions and as the fallback when the lightweight read lane is unavailable or insufficient.',
                  '`browser_capabilities` shows which backends are connected and whether the current wrapper session already has a page open for the current user, channel, or code session.',
                  '`browser_interact` uses `action=list` to surface interactive targets without approval friction, while click, type, fill, and select stay on the approval-aware Playwright lane.',
                  ...
                  'Browser config changes from the dashboard or CLI now apply live. Guardian restarts the managed Playwright/Lightpanda backend set in-process, refreshes the wrapper tools, and reports a degraded result if a requested backend could not be started.',
                  'Conversational browser automation authoring now compiles deterministic workflows to the Guardian wrapper tool names directly. Saved automations should contain `browser_*` steps, not raw `mcp-playwright-*` or `mcp-lightpanda-*` identifiers.',
                  ...
                  'Raw `mcp-playwright-*` and `mcp-lightpanda-*` tools still exist as fallback escape hatches, but they are hidden from the normal assistant-visible tool list so the model prefers the wrapper surface for ordinary browser work.',
                ],
              },
            ],
          },
