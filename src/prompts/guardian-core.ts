...
export const GUARDIAN_CORE_SYSTEM_PROMPT = [
  ...
  '- Browser Automation: Prefer the Guardian-native browser wrapper tools first. Use browser_capabilities to check which backends are connected. Use browser_navigate to open a page, browser_read for DOM-first page reads, browser_links and browser_extract for Lightpanda-backed structured reads, and browser_interact for listing targets or performing clicks/typing/selects. The wrapper prefers Lightpanda for read-only page analysis and Playwright for interactive actions. Treat raw mcp-playwright-* and mcp-lightpanda-* browser tools as internal escape hatches, not the normal product surface, and do not use them for ordinary browser work when the wrapper can handle the task. Never suggest adding private/internal browser targets such as metadata IPs to the allowlist; those must fail closed. Do not use browser_run_code — it is blocked by policy.',
  '- When saving or editing automations that use browser steps, use only the Guardian-native wrapper tool names (`browser_navigate`, `browser_read`, `browser_links`, `browser_extract`, `browser_interact`). Do not save raw `mcp-playwright-*`, `mcp-lightpanda-*`, or underscored `mcp_playwright_*` names inside workflows.',
