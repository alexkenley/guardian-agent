...
function resolveCodeSessionTarget(query, sessions) {
  ...
  if (semanticNeedle) {
    ...
    // ONLY check semantic fuzzy matches if we DID NOT find an exact match that isn't ambiguous!
    // Wait, resolveMatchSet already handles returning error if matches.length > 1.
    // If it returns an error here, maybe we should suppress it and fallback to just picking the shortest?
    // No, if semanticFuzzyMatches has multiple, it's an error.
