...
3. **Review tool policy** — defaults to `on-request` / `approve_each` for the main assistant, with a read-only shell allowlist. Mutating tools still require approval, and approved workspace-local package installs are tracked per workspace instead of being stored in global memory.
